...
def get_pie_chart(entered_site):
    if not entered_site:
        return px.pie(labels=['No Data'], values=[1], title='No Data Available')

    if entered_site == 'ALL':
        filtered_df = spacex_df.groupby(['Launch Site'], as_index=False)['class'].mean()
        filtered_df['Launch Site'] = 'All Sites'
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
    
    counts = filtered_df['class'].value_counts().to_dict()
    outcomes_df = pd.DataFrame(list(counts.items()), columns=['Outcome', 'Count'])

    fig = px.pie(outcomes_df, values='Count', names='Outcome', title=f'Launch Success Rate For {entered_site}')
